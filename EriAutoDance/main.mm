...
// Đoạn script Lua nhúng trực tiếp
...
nativeLog("AutoDance HexControl v4 da duoc nhung truc tiep va nap thanh cong!")
)lua";

static lua_State *gL = NULL;

__attribute__((constructor)) static void initLua() {
    @autoreleasepool {
        gL = luaL_newstate();
        if (gL) {
            luaL_openlibs(gL);
